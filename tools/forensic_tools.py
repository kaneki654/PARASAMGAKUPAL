...
class Autopsy(HackingTool):
    TITLE = "Autopsy - Autopsy is a platform that is used by Cyber Investigators"
...
class Wireshark(HackingTool):
    TITLE = "Wireshark - Network captureWireshark is a network capture and analyzer and analyzer"
...
class BulkExtractor(HackingTool):
    TITLE = "Bulk extractor - xtract useful information without parsing the file system"
...
class Guymager(HackingTool):
    TITLE = "Disk Clone and ISO Image Aquire - Guymager is a free forensic imager for media acquisition"
...
class Toolsley(HackingTool):
    TITLE = "Toolsley - Tools for investigation"
